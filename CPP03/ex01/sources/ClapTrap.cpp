...
/*   Updated: 2025/03/25 14:43:23 by mrahmat-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ClapTrap::attack(const std::string& target, const std::string& trap)
{
	if (_hit_points > 0)
	{
		if (_energy_points > 0)
		{
			std::cout << trap << " " << _name << " attacks " << target << ", causing " << _attack_damage << " points of damage!" << std::endl;
			_energy_points--;
		}
		else
			std::cout << trap << " " << _name << " cannot attack and is in desperate need of energy!" << std::endl;
	}
	else
...
void	ClapTrap::takeDamage(unsigned int amount, const std::string& trap)
{
	if (_hit_points > 0)
	{
		if (_hit_points > amount)
		{
			std::cout << trap << " " << _name << " has taken " << amount << " amount of damage and has only " << _hit_points - amount << " hit points left!" << std::endl;
			_hit_points -= amount;
		}
		else
		{
			std::cout << trap << " " << _name << " has taken " << amount << " amount of damage and has died!" << std::endl;
			_hit_points = 0;
		}
	}
	else
		std::cout << trap << " " << _name << " is dead and beyond repair." << std::endl;
}
...
void	ClapTrap::beRepaired(unsigned int amount, const std::string& trap)
{
	if (_hit_points > 0)
	{
		if (_energy_points > 0)
		{
			std::cout << trap << " " << _name << " has repaired itself by " << amount << " amount and now has " << _hit_points + amount << " hit points!" << std::endl;
			_hit_points += amount;
			_energy_points--; 
		}
		else
			std::cout << trap << " " << _name << " cannot repair itself and is in desperate need of energy!" << std::endl;
	}
	else
		std::cout << trap << " " << _name << " is dead and beyond repair." << std::endl;
}
